...
def parse_args():
    ...
    parser.add_argument('--name', type=str, required=True,
                        help="[1] name of one bird species; [2] file of bird species spaced by '\\n' ")
    parser.add_argument('--process-ratio', type=float, default=0.8,
                        help="float[0~1], define cpu utilities in downloading audios [default: 0.8]")
...
def metadata(filt):
    ...
    for f in filt:
        ...
        filt_path.append((f.replace(' ', '_')).replace(':', '_').replace("\"", "").replace(' ', '_').replace("\'", ""))
    ...
    for fu in tqdm(filt_url, desc="retrieving metadata"):
        print(fu)
        path = 'dataset/metadata/' + fu.replace('%20', '_').replace(':', '_').replace("\"", "").replace(' ', '_').replace("\'", "").replace('ü', 'u')
        page, page_num = 1, 1
        while page < page_num + 1:
            url = 'https://www.xeno-canto.org/api/2/recordings?query={0}&page={1}'.format(fu.replace(' ', ''), page)
            try:
...
def download(pid, pTotal, metadata_paths):
    ...
    for pa in paths:
        ...
        if int(data['numRecordings']) > 0:
            bird_name = data['recordings'][0]['en'].replace(' ', '_').replace('-', '_').replace("\"", "").replace("\'", "").lower()
    ...
    for curr_rec in tqdm(all_recordings, desc="process %d" % os.getpid()):
        ...
        url = curr_rec['file']
        name = (curr_rec['en']).replace('-', '_').replace(' ', '_').replace("\"", "").replace("\'", "").lower()
        ...
        if int(track_id) in redown:
            print("File " + str(
                track_id) + ".mp3 must be redownloaded since it was not completed during a previous query.")
