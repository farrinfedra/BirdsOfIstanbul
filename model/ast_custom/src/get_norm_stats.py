...
data_loader = torch.utils.data.DataLoader(
    dataloader_custom.AudiosetDataset('/kuacc/users/bbiner21/ast/egs/custom/data/datafiles_large_22/train_data_1.json', label_csv='/kuacc/users/bbiner21/ast/egs/custom/bc-2022/bc_custom_labels_large.csv',
                                audio_conf=audio_conf), batch_size=96, shuffle=False, num_workers=8, pin_memory=True)



# second_loader = torch.utils.data.DataLoader(
#     dataloader_custom.AudiosetDataset('/kuacc/users/bbiner21/ast/egs/custom/data/datafiles/eval1.json', label_csv='/kuacc/users/bbiner21/ast/egs/custom/data/custom_labels.csv',
#                                 audio_conf=audio_conf), batch_size=96, shuffle=False, num_workers=8, pin_memory=True)
...
# for i, (audio_input, labels) in enumerate(second_loader):
#     cur_mean = torch.mean(audio_input)
#     cur_std = torch.std(audio_input)
#     mean.append(cur_mean)
#     std.append(cur_std)
