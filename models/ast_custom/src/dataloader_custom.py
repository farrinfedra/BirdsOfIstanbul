...
#             self.BIRD_CODE[label] = i
#             self.INV_BIRD_CODE[i] = label
#         self.BIRD_CODE['rocpig1'] = len(primary_labels)

#         self.NOCALL_CODE = self.BIRD_CODE['nocall']
...
#         plt.show(block=False)
    ...
    def get_soundscape_clip(self,data_path, sample, period, train_aug):
        input_path = Path('/datasets/birdclef-2021/train_soundscapes')
        
        
        filename = sample['wav'].split('/')[-1]
        ...
        #clip = [period*segment:period*(segment+1)]
        clip = record[period*segment:period*(segment+1)]
        
        #period = int(np.ceil(5 * sr))
        return clip

    def get_sample_clip(self,data_path, sample, period, train_aug):
        filename = sample['wav']
